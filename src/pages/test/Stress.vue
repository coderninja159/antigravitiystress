...
import { PhArrowLeft, PhArrowRight, PhCheckCircle } from '@phosphor-icons/vue'
...
<template>
  <div class="max-w-3xl mx-auto w-full flex-grow flex flex-col justify-center py-8 px-4 relative">
    ...
    <div v-show="store.loading" class="absolute inset-0 z-50 flex flex-col items-center justify-center bg-light-bg/80 dark:bg-dark-bg/80 backdrop-blur-sm">
      <div class="animate-spin rounded-full h-16 w-16 border-b-4 border-accent mb-4"></div>
    ...
    <div v-if="currentQuestion && !store.loading" class="w-full">
      ...
      <div class="mb-10">
        ...
        <div class="w-full bg-light-border dark:bg-dark-border rounded-full h-3 overflow-hidden">
          <div class="bg-accent h-3 rounded-full transition-all duration-500 ease-out" :style="{ width: `${((currentQIndex) / totalQuestions) * 100}%` }"></div>
        </div>
      </div>
      ...
      <div ref="questionContainer" class="bg-white dark:bg-dark-card rounded-3xl shadow-lg border border-light-border dark:border-dark-border p-8 sm:p-12 relative">
        ...
        <div class="flex flex-col gap-4">
          <button 
            ...
            :class="selectedOption === opt.id ? 'border-accent bg-accent/5 dark:bg-accent/10 shadow-sm' : 'border-light-border dark:border-dark-border hover:border-accent/50 dark:hover:border-accent/40 hover:bg-gray-50 dark:hover:bg-gray-800/50'"
          >
            <div class="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-base shrink-0 transition-colors"
                 :class="selectedOption === opt.id ? 'bg-accent text-white shadow-md shadow-accent/30' : 'bg-gray-100 text-light-muted dark:bg-[#1f242b] dark:text-dark-muted group-hover:bg-accent/20 group-hover:text-accent'">
              {{ getLetter(idx) }}
            </div>
        ...
        <div class="mt-12 flex justify-between items-center">
          ...
          <Button @click="nextOrFinish" :disabled="!allowNext" class="bg-accent hover:bg-[#C0764A] shadow-accent/30">
            {{ currentQIndex === totalQuestions - 1 ? t('test.finish') : t('test.next') }}
            <PhArrowRight v-if="currentQIndex !== totalQuestions - 1" :size="20" />
            <PhCheckCircle v-else :size="20" />
          </Button>
        </div>
      </div>
    </div>
